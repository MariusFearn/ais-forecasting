...
class TFTModel(BaseTimeSeriesModel):
    ...
    Wraps PyTorch Forecasting's TemporalFusionTransformer with our project interface.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize the TFT model with the given configuration.
        ...
        self.hidden_continuous_size = config.get("hidden_continuous_size", 16)
        self.loss = config.get("loss", QuantileLoss())
    ...
    def fit(self, train_dataloader=None, val_dataloader=None, df_data=None):
        ...
        logger = pl.loggers.TensorBoardLogger(
            ...
            name=self.config.get("experiment_name", "tft_model"),
        )
    ...
    def load(cls, path: str, config: Dict[str, Any], training_dataset=None):
        ...
        Returns:
            TFTModel: Loaded model instance
        """
        instance = cls(config)
        
        if training_dataset is None:
            raise ValueError("training_dataset must be provided to load a TFT model")
        ...
        instance.model = TemporalFusionTransformer.from_dataset(
            ...
            output_size=instance.max_prediction_length,
            loss=instance.loss,
        )
