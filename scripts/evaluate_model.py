...
class UnifiedModelEvaluator:
    ...
    def _prepare_test_data(self, test_data: pd.DataFrame, h3_encoder) -> tuple:
        """Prepare test features and targets, handling unseen H3 cells."""
        # Prepare features and targets
        feature_cols = [col for col in test_data.columns 
                      if col not in ['target_h3_cell', 'vessel_imo']]
        
        # Handle unseen H3 cells in test set
        print(f"   📊 Filtering test data for known H3 cells...")
        known_cells = set(h3_encoder.classes_)
        test_mask = test_data['target_h3_cell'].isin(known_cells)
        
        print(f"   📉 Filtering: {len(test_data)} → {test_mask.sum()} samples (removed {(~test_mask).sum()} unseen cells)")
        
        X_test = test_data[test_mask][feature_cols].copy()
        y_test = h3_encoder.transform(test_data[test_mask]['target_h3_cell'])
    ...
    def _simple_evaluation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Prepare test data with unseen cell filtering
            X_test, y_test, filtered_test_data = self._prepare_test_data(test_data, h3_encoder)
            
            # Clean data
            X_test = self._clean_features(X_test)
            
            # Apply feature selection if exists
            X_test = self._apply_feature_selection(X_test, "comprehensive")
    ...
    def _comprehensive_evaluation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            vessel_performance = {}
            if 'vessel_imo' in filtered_test_data.columns:
                vessel_performance = self._performance_by_vessel(
                    filtered_test_data, y_test, y_pred, h3_encoder
                )
    ...
    def _apply_feature_selection(self, X: pd.DataFrame, model_name: str = "comprehensive") -> pd.DataFrame:
        ...
        if selector_path.exists():
            try:
                ...
                print(f"   ✅ Feature selection applied: {X.shape[1]} features selected from {len(feature_selector.get_support())}")
            except Exception as e:
                print(f"   ⚠️  Feature selection failed: {e}")
        else:
            print(f"   ⚠️  No feature selector found at: {selector_path}")
